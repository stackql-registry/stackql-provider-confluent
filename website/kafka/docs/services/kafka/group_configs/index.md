...
<tr><td><b>Id</b></td><td><CopyableCode code="kafka.kafka.group_configs" /></td></tr>
</tbody></table>
...
synonyms,
value
FROM kafka.kafka.group_configs
...
```sql
REPLACE kafka.kafka.group_configs
SET 
value = '{{ value }}'
WHERE 
...
```sql
DELETE FROM kafka.kafka.group_configs
WHERE cluster_id = '{{ cluster_id }}' --required
AND group_id = '{{ group_id }}' --required
AND name = '{{ name }}' --required
AND kafka_endpoint_id = '{{ kafka_endpoint_id }}' --required
...
```sql
EXEC kafka.kafka.group_configs.update_kafka_group_config_batch 
@cluster_id='{{ cluster_id }}' --required, 
